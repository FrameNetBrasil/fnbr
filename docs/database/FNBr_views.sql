...
select vp.idValencePattern, vp.idFrame, vp.idLU, vp.idLanguage, vp.idPattern, vp.countPattern, vp.idFrameElement, vp.GF, vp.GFSource, vp.PT, 
  lu.name as luName, fentry.name as frameName, feentry.name as feName, l.language
from ValencePattern vp
join Frame f on (vp.idFrame = f.idFrame)
join FrameElement fe on (vp.idFrameElement = fe.idFrameElement)
join LU on (vp.idLU = lu.idLU)
join Language l on (vp.idLanguage = l.idLanguage)
join entry fentry on (f.entry = fentry.entry)
join entry feentry on (fe.entry = feentry.entry)
where (fentry.idLanguage = vp.idLanguage)
and (feentry.idLanguage = vp.idLanguage);
...
d.entry as documentEntry, d.author, d.idGenre, d.idCorpus, c.entry as entryCorpus
from sentence s 
