...
class StructureLemmaService extends MService
{
    ...
    public function listLemmaLexeme($idLemma, $idLanguage)
    {
        ...
        foreach ($lexemes as $lexeme) {
            ...
            $node['text'] = $lexeme['name'] . '.' . strtolower($lexeme['POS']);
            $node['state'] = 'closed';
    ...
    public function listConstraintsFE($idLemmaElement, $idLanguage)
    {
        $service = Manager::getAppService('structureconstraints');
        $result = $service->listConstraintsFE($idLemmaElement);
        return $result;
    }
...
//            $field = 'wf' . $word[0];
    ...
    public function deleteLemma($idLemma)
    {
        ...
        try {
            ...
            $filter = (object) ['idLemma' => $idLemma];
            $fes = $lemmaElement->listByFilter($filter)->asQuery()->getResult();
            foreach ($fes as $fe) {
    ...
    public function addConstraintsFE($data)
    {
        try {
            ...
            if ($data->idLemma != '') {
                ...
                $cf = new fnbr\models\LemmaElement($data->idLemmaElement);
                $lemma = new fnbr\models\Lemma($data->idLemma);
                Base::createEntityRelation($constraint->getIdEntity(), 'rel_constraint_lemma', $cf->getIdEntity(), $lemma->getIdEntity());
            }
            ...
            if ($data->idFEQualia != '') {
                $fe = new fnbr\models\LemmaElement($data->idLemmaElement);
                $feQualia = new fnbr\models\LemmaElement($data->idFEQualia);
                Base::createEntityRelation($fe->getIdEntity(), $data->relation, $feQualia->getIdEntity());
            }
            if ($data->idFEMetonymy != '') {
                $fe = new fnbr\models\LemmaElement($data->idLemmaElement);
                $feMetonymy = new fnbr\models\LemmaElement($data->idFEMetonymy);
